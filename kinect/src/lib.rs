...
impl Kinect {
    pub fn new() -> Result<Self, std::io::Error> {
        unsafe fn load_rekinect_dyn(lib: &str) -> Result<Kinect, std::io::Error> {
            type GmKinectDynInit =
                unsafe extern "Rust" fn() -> Result<Box<dyn KinectBackend>, std::io::Error>;
        ...
        macro_rules! try_load_backend {
            ($backend:literal) => {
                match {
                    unsafe { load_rekinect_dyn(concat!("garrysmod/bin/", $backend)) }
                        .or_else(|_| unsafe { load_rekinect_dyn($backend) })
                } {
        ...
        Err(std::io::Error::new(std::io::ErrorKind::Unsupported, "No backend available, did you remember to install one? https://github.com/WilliamVenner/gm_rekinect"))
    }
