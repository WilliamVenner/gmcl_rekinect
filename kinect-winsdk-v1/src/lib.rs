...
extern "C" {
    fn WinSdkKinectV1_Create(
        callback: CWinSdkKinectV1Callback,
        userdata: *mut c_void,
    ) -> *mut c_void;
    ...
    fn pos(&self) -> Option<WinSdkKinectV1Skeleton> {
        match self.state {
            ...
            SkeletonTrackingState::PositionOnly => {
                Some(WinSdkKinectV1Skeleton::PositionOnly(unsafe {
                    self.pos.assume_init_ref().pos_only
                }))
            }
            SkeletonTrackingState::Tracked => Some(WinSdkKinectV1Skeleton::Tracked(unsafe {
...
impl<U> Drop for WinSdkKinectV1<U> {
    fn drop(&mut self) {
        ...
        unsafe {
            PostThreadMessageW(
                GetThreadId(HANDLE(thread.as_raw_handle() as isize)),
                WM_QUIT,
                WPARAM(0),
                LPARAM(0),
            )
...
pub extern "Rust" fn gm_kinect_init() -> Result<Box<dyn KinectBackend>, std::io::Error> {
    extern "C" fn callback(
        event: WinSdkKinectV1SkeletonUpdate,
        tx: &mut std::sync::mpsc::SyncSender<WinSdkKinectV1SkeletonUpdate>,
    ) {
        tx.send(event).ok();
    }
        ...
        fn poll(&mut self) -> Option<KinectSkeleton> {
            ...
            if self.skeleton.is_none() || self.skeleton == Some(event.skeleton_index) {
                if let Some(WinSdkKinectV1Skeleton::Tracked(pos)) = event.pos() {
                    ...
                    pos.raw_bones()
                        .iter()
                        .zip(raw_bones.iter_mut())
                        .for_each(|(src, dst)| {
                            *dst = [src.x, src.y, src.z];
                        });

                    return Some(KinectSkeleton::Tracked(
                        KinectTrackedSkeleton::from_raw_bones(raw_bones),
                    ));
